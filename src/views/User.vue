<template>
  <div>
    <v-card v-if="user" max-width="600" class="mx-auto mt-2 pa-4">
      <v-img
        contain
      ...
      <v-card-subtitle v-if="user_is_current_user"> it's you! </v-card-subtitle>
      ...
      <v-list>
        ...
        <v-list-item v-if="current_user_is_admin || user_is_current_user">
          <v-list-item-content>
            <v-text-field label="Avatar URL" v-model="user.avatar_src" />
          </v-list-item-content>
        </v-list-item>
    ...
    <v-snackbar v-model="snack.show" :color="snack.color">
      ...
      <template v-slot:action="{ attrs }">
        <v-btn icon v-bind="attrs" @click="snack.show = false">
...
export default {
  ...
  methods: {
    ...
    update_user() {
      ...
      this.axios
        .patch(url, this.modified_properties)
        .then(() => {
