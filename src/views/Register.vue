<template>
  <v-card class="mx-auto py-3 mt-12" max-width="500px">
    ...
    <template v-if="!processing && success">
      <v-card-text>
        Account registration successfull. An email has been sent to your address
        for activation.
      </v-card-text>
      <v-card-text>
        Once the activation process is complete, click
        <router-link :to="{ name: 'login' }">here</router-link> to login
      </v-card-text>
    </template>
...
export default {
  ...
  data() {
    return {
      ...
      emailRules: [
        (v) =>
          !!v || this.current_user_is_admin || "e-mail address is required",
        (v) =>
          (!!v && v.length < 50) ||
          this.current_user_is_admin ||
          "email_address must be less than 50 characters",
        (v) =>
